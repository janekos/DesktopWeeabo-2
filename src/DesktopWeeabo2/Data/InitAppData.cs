...
namespace DesktopWeeabo2.Data {
	...
	public static class InitAppData {
		...
		private static void CheckFiles() {
			if (!CheckRootDir())
				Directory.CreateDirectory(GlobalConfig.AppDir);
			if (!File.Exists(GlobalConfig.AppDir + "\\entries.db"))
				DbActions.InitDB();
			if (!File.Exists(GlobalConfig.AppDir + "\\config.json"))
				GlobalConfig.SerializeConfig();
		}
	}
}